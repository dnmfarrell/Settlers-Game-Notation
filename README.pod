...
A complete example game log of v0.4 SGN is included in this repo, here: `test-corpus/game-log-v0.4.json`.

=head2 Tests

The F<t> directory has test scripts in Perl and JavaScript. F<test-corpus/messages.json> contains example valid and invalid messages used for testing. The Perl tests can be run from the root project directory with `prove`, which require L<JSON::Validator|https://metacpan.org/pod/JSON::Validator> and L<JSON::XS|https://metacpan.org/pod/JSON::XS>:
...
The F<schema/v0.4/message.json> file is a JSON schema for an SGN message.
